...
import { updateFilm } from '../../../services/endpoints/film';
...
const FilmEdit = ({ onNavigate, selectedFilm }) => {
  ...
  const [posterPreview, setPosterPreview] = useState(selectedFilm?.poster ? getStorageUrl(selectedFilm.poster) : '');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await updateFilm(selectedFilm.id, data);
      alert('Film berhasil diupdate!');
      onNavigate('film-list');
    } catch (error) {
      console.error('Error updating film:', error);
      alert(error.response?.data?.message || 'Gagal mengupdate film');
    } finally {
      setLoading(false);
    }
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <label className="flex items-center justify-center w-full px-4 py-8 bg-gray-700 border-2 border-dashed border-gray-600 rounded-lg cursor-pointer hover:bg-gray-600 transition">
              <div className="text-center">
                ...
                <span className="block text-xs text-gray-500 mt-1">PNG, JPG, JPEG (Max 2MB) - Kosongkan jika tidak ingin mengubah</span>
              </div>
              <input
                type="file"
                accept="image/*"
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4 pt-4">
            ...
            >
              ...
              <span>{loading ? 'Menyimpan...' : 'Update Film'}</span>
            </button>
            <button
              type="button"
